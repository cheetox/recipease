...
#def search():
  ...
  #      most_popular = recommenders.sample_popular()
   #     hrs=request.form['tst']
    #    df=word2vec_rec.get_recs(hrs)
     #   return render_template('quiz.html',tables=[df.to_html(classes='data')], titles=df.columns.values,most_popular = ((most_popular,[utils.get_url(utils.title_to_id(recipe)) for recipe in most_popular])))
...
def quiz():
    ...
    if request.method == "POST" and "tst" in request.form:
        #hrs=request.form['tst']
        #print(type(tst))
        most_popular = recommenders.sample_popular()
        hrs=request.form['tst']
